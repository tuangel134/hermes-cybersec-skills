---
name: cybersec-toolkit-llm
description: Selecciona y organiza herramientas del módulo llm de cybersec-toolkit (9 tools upstream). Úsalo en labs autorizados cuando el usuario pida LLM red teaming, prompt injection, jailbreak testing and AI vulnerability scanning.
---

# LLM Red Teaming / Prompt Injection / AI Security

Módulo upstream: `llm`  
Conteo upstream declarado: **9 tools**  
Uso: LLM red teaming, prompt injection, jailbreak testing and AI vulnerability scanning.
...
```bash
cd cybersec-toolkit
sudo ./install.sh --dry-run --module llm
sudo ./install.sh --module llm
sudo ./scripts/verify.sh --module llm --skip-heavy
```

## Herramientas seed incluidas en este skill

- Vigil, garak, FuzzyAI, promptfoo, pallms, shannon, cai-framework
- pentagi, pentagi-docker

## Selección rápida

- Para instalar solo una herramienta: `sudo ./install.sh --tool <nombre>`.
- Para evitar herramientas pesadas: `sudo ./install.sh --module llm --skip-heavy`.
- Para consultar estado con MCP: `get_module_info("llm")` y `check_installed("<tool>")`.
...
Input: "Instala lo necesario del módulo llm"  
Output: Ejecutar primero `sudo ./install.sh --dry-run --module llm`, revisar dependencias y luego instalar si el usuario confirma.

Input: "Qué herramientas de llm usaría para mi lab"  
