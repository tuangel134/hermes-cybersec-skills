---
name: cybersec-toolkit-reversing
description: Selecciona y organiza herramientas del módulo reversing de cybersec-toolkit (31 tools upstream). Úsalo en labs autorizados cuando el usuario pida Disassemblers, debuggers, emulation and reverse engineering.
---

# Reversing / Debugging / Emulation

Módulo upstream: `reversing`  
Conteo upstream declarado: **31 tools**  
Uso: Disassemblers, debuggers, emulation and reverse engineering.
...
```bash
cd cybersec-toolkit
sudo ./install.sh --dry-run --module reversing
sudo ./install.sh --module reversing
sudo ./scripts/verify.sh --module reversing --skip-heavy
```

## Herramientas seed incluidas en este skill

- radare2, ghidra, checksec, rizin, gdb, binwalk, binutils
- qemu-user-static, qemu-system-x86, valgrind, rr, ltrace, strace, hexedit
- upx-ucl, nasm, edb-debugger, angr, frida-tools, uncompyle6, pwndbg
- GEF, peda, decomp2dbg, Qiling, Krakatau, pyinstxtractor, ELFkickers
- rappel, rp-lin, jd-gui

## Selección rápida

- Para instalar solo una herramienta: `sudo ./install.sh --tool <nombre>`.
- Para evitar herramientas pesadas: `sudo ./install.sh --module reversing --skip-heavy`.
- Para consultar estado con MCP: `get_module_info("reversing")` y `check_installed("<tool>")`.
...
Input: "Instala lo necesario del módulo reversing"  
Output: Ejecutar primero `sudo ./install.sh --dry-run --module reversing`, revisar dependencias y luego instalar si el usuario confirma.

Input: "Qué herramientas de reversing usaría para mi lab"  
