---
name: cybersec-toolkit-stego
description: Selecciona y organiza herramientas del módulo stego de cybersec-toolkit (13 tools upstream). Úsalo en labs autorizados cuando el usuario pida Image/audio steganography, detection and StegCracker-style workflows.
---

# Stego / Image / Audio / Hidden Data

Módulo upstream: `stego`  
Conteo upstream declarado: **13 tools**  
Uso: Image/audio steganography, detection and StegCracker-style workflows.
...
```bash
cd cybersec-toolkit
sudo ./install.sh --dry-run --module stego
sudo ./install.sh --module stego
sudo ./scripts/verify.sh --module stego --skip-heavy
```

## Herramientas seed incluidas en este skill

- steghide, stegsnow, pngcheck, sonic-visualiser, exiv2, pngtools, stegoveritas
- zsteg, stegsolve, openstego, stegextract, stegosaurus, stegseek

## Selección rápida

- Para instalar solo una herramienta: `sudo ./install.sh --tool <nombre>`.
- Para evitar herramientas pesadas: `sudo ./install.sh --module stego --skip-heavy`.
- Para consultar estado con MCP: `get_module_info("stego")` y `check_installed("<tool>")`.
...
Input: "Instala lo necesario del módulo stego"  
Output: Ejecutar primero `sudo ./install.sh --dry-run --module stego`, revisar dependencias y luego instalar si el usuario confirma.

Input: "Qué herramientas de stego usaría para mi lab"  
